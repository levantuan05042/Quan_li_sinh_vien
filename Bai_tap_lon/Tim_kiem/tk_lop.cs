...
namespace Bai_tap_lon
{
    public partial class tk_lop : Form
    {
        ...
        {
            // TODO: This line of code loads data into the 'qL_SinhVienDataSet16.Lop' table. You can move, or remove it, as needed.
            //this.lopTableAdapter.Fill(this.qL_SinhVienDataSet16.Lop);
            txt_tk.Focus();
            rad_ma.Checked= true;
        }
        void loaddata()
        ...
        private void txt_tk_TextChanged(object sender, EventArgs e)
        {
            ...
            if (rad_ma.Checked)
            {
                command.CommandText = "SELECT * FROM Lop WHERE ma_lop like N'%" + txt_tk.Text + "%'";
            }
            else if (rad_ten.Checked)
            {
                command.CommandText = "SELECT * FROM Lop WHERE ten_lop like N'%" + txt_tk.Text + "%'";
            }
            else if (rad_makhoa.Checked)
            {
                command.CommandText = "SELECT * FROM Lop WHERE ma_khoa like N'%" + txt_tk.Text + "%'";
            }
            adapter.SelectCommand = command;
            table.Clear();
            adapter.Fill(table);
        ...
        private void btn_timkiem_Click(object sender, EventArgs e)
        {
            ...
            if (rad_ma.Checked)
            {
                command.CommandText = "SELECT * FROM Lop WHERE ma_lop = @searchValue";
            }
            else if (rad_ten.Checked)
            {
                command.CommandText = "SELECT * FROM Lop WHERE ten_lop = @searchValue'";
            }
            else if (rad_makhoa.Checked)
            {
                command.CommandText = "SELECT * FROM Lop WHERE ma_khoa = @searchValue";
            }
        ...
        private void rad_makhoa_CheckedChanged(object sender, EventArgs e)
        {
            txt_tk_TextChanged(sender, e);
        }
